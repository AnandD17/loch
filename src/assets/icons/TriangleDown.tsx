import React from 'react'
import { InterfaceSVG } from '../../interfaces/interfaces'

const TriangleDown = (props: InterfaceSVG) => {
  return (
    <svg width={props.width||"12"} height={props.height||"12"} viewBox={`0 0 ${props.width||12} ${props.height||12}`} fill={props.fill||"none"} xmlns="http://www.w3.org/2000/svg">
    <path d="M3.79965 4.59953L6.17719 6.97704C6.24553 7.04538 6.35633 7.04538 6.42467 6.97704L8.80215 4.59953C8.9124 4.48928 8.83435 4.30078 8.67839 4.30078H3.9234C3.76749 4.30078 3.68941 4.48928 3.79965 4.59953Z" fill="#96979A"/>
    </svg>
    
  )
}

export default TriangleDown